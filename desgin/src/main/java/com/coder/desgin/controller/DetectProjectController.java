...
public class DetectProjectController {
    ...
    public String deleteProjectList(String userId,String email, String token, String validationInfo,List<String> detectIds) {
        if (tokenUtil.verify(token)) {
            if (!verificationCodeFactory.checkValidationInfo(email, validationInfo, Constants.DELETE) ) {
                ...
                // 验证邮箱和删除记录的相关性
                if(!userService.checkAccountAndEmail(new User(userId, email))) {
                    return RespMessageUtils.ERROR("您邮箱对应的账号出现异常, 账号已经冻结, 请注意检测邮箱是否泄露!");
                } else if (projectService.confirmOwnership(userId, detectIds)) {
